...
function createPopup()
    return WindUI:Popup({
        ...
        Content = "Hello!",
        Buttons = {
            { Title = "Hahaha", Icon = "bird", Variant = "Tertiary" },
            { Title = "Hahaha", Icon = "bird", Variant = "Tertiary" },
            { Title = "Hahaha", Icon = "bird", Variant = "Tertiary" },
        },
    })
...
local Window = WindUI:CreateWindow({
    ...
    NewElements = true,
    HideSearchBar = false,
    OpenButton = {        Title = "Open .ftgs hub UI",
...
local function parseJSON(luau_table, indent, level, visited)
    ...
    if dataType == "table" then
        ...
        local isArray = true
        local maxIndex = 0        for k, _ in pairs(luau_table) do
        ...
        if isArray then
            ...
            table.sort(keys, function(a, b)
                if type(a) == type(b) then return tostring(a) < tostring(b)
    ...
    elseif dataType == "string" then
        ...
        escaped = escaped:gsub("\t", "\\t")
        return '"' .. escaped .. '"'    elseif dataType == "number" then return tostring(luau_table)
...
do
    ...
    AboutSection:Section({ Title = "What is WindUI?", TextSize = 24, FontWeight = Enum.FontWeight.SemiBold })
    AboutSection:Space()
    ...
    AboutTab:Button({
        ...
        Icon = "",        Callback = function()
            ...
            WindUI:Notify({ Title = "WindUI JSON", Content = "Copied to Clipboard!" })
        end,
    })
...
do
    ...
    local OverviewSection1 = OverviewGroup3:Section({ Title = "Section 1", Desc = "Section example", Box = true, BoxBorder = true, Opened = true })
    ...
    local OverviewSection2 = OverviewGroup3:Section({ Title = "Section 2", Box = true, BoxBorder = true, Opened = true })
...
do
    ...
    ToggleTab:Space()
    ToggleTab:Toggle({ Title = "Toggle", Desc = "Toggle example" })
    ToggleTab:Space()
    ...
    ToggleTab:Space()
    ToggleTab:Toggle({ Title = "Checkbox", Desc = "Checkbox example", Type = "Checkbox" })
    ToggleTab:Space()
    ToggleTab:Toggle({ Title = "Toggle", Locked = true, LockedTitle = "This element is locked" })
    ToggleTab:Toggle({ Title = "Toggle", Desc = "Toggle example", Locked = true, LockedTitle = "This element is locked" })
end
...
do
    ...
    HighlightButton = ButtonTab:Button({
        ...
        Icon = "mouse",
        Callback = function() print("clicked highlight"); HighlightButton:Highlight() end,
    })
    ButtonTab:Space()
    ButtonTab:Button({ Title = "Blue Button", Color = Color3.fromHex("#305dff"), Icon = "", Callback = function() end })
    ButtonTab:Space()
    ButtonTab:Button({ Title = "Blue Button", Desc = "With description", Color = Color3.fromHex("#305dff"), Icon = "", Callback = function() end })
    ButtonTab:Space()
    ButtonTab:Button({
        Title = "Notify Button",
        Callback = function()
    ...
    ButtonTab:Space()
    ButtonTab:Button({ Title = "Button", Locked = true, LockedTitle = "This element is locked" })
    ButtonTab:Button({ Title = "Button", Desc = "Button example", Locked = true, LockedTitle = "This element is locked" })
end
...
do
    ...
    InputTab:Space()    InputTab:Input({ Title = "Input Textarea", Type = "Textarea", Icon = "mouse" })
    InputTab:Space()
    ...
    InputTab:Space()
    InputTab:Input({ Title = "Input", Locked = true, LockedTitle = "This element is locked" })
    InputTab:Input({ Title = "Input", Desc = "Input example", Locked = true, LockedTitle = "This element is locked" })
end
...
do
    ...
    SliderTab:Section({ Title = "Default Slider with Tooltip", TextSize = 14 })
    SliderTab:Slider({
    ...
    SliderTab:Slider({
        IsTooltip = true, Step = 1,
        Value = { Min = 0, Max = 200, Default = 100 },
        Icons = { From = "sfsymbols:sunMinFill" },        Callback = function(value) print(value) end,
    })
    SliderTab:Space()
    SliderTab:Slider({
        IsTooltip = true, Step = 1,
...
do
    ...
    DropdownTab:Dropdown({
        ...
        Values = {
            { Title = "New file", Desc = "Create a new file", Icon = "file-plus", Callback = function() print("Clicked 'New File'") end },
            { Title = "Copy link", Desc = "Copy the file link", Icon = "copy", Callback = function() print("Clicked 'Copy link'") end },
            { Title = "Edit file", Desc = "Allows you to edit the file", Icon = "file-pen", Callback = function() print("Clicked 'Edit file'") end },
            { Type = "Divider" },
            { Title = "Delete file", Desc = "Permanently delete the file", Icon = "trash", Callback = function() print("Clicked 'Delete file'") end },
        },
    })
    DropdownTab:Space()
    DropdownTab:Dropdown({
        Title = "Multi Dropdown",
        Values = { "Привет", "Hello", "Сәлем", "Bonjour" },
        Value = nil, AllowNone = true, Multi = true,
        Callback = function(selectedValue) print("Selected: " .. selectedValue) end,
    })
    DropdownTab:Space()
    DropdownTab:Dropdown({
...
if not RunService:IsStudio() and writefile and printidentity then
    do        local ConfigElementsTab = ConfigUsageSection:Tab({
        ...
        ConfigElementsTab:Colorpicker({
            Flag = "ColorpickerTest", Title = "Colorpicker", Desc = "Colorpicker Description",
            Default = Color3.fromRGB(0, 255, 0), Transparency = 0, Locked = false,
            Callback = function(color) print("Background color: " .. tostring(color)) end,
        })
        ConfigElementsTab:Space()
        ConfigElementsTab:Dropdown({
            Flag = "DropdownTest", Title = "Advanced Dropdown",
            Values = {
            ...
            Value = "Category A",
            Callback = function(option) print("Category selected: " .. option.Title) end,
        })
        ConfigElementsTab:Dropdown({
            Flag = "DropdownTest2", Title = "Advanced Dropdown 2",
            Values = {
                { Title = "Category A", Icon = "bird" },
                { Title = "Category B", Icon = "house" },
                { Title = "Category C", Icon = "droplet", Locked = true },
            },
            Value = "Category A", Multi = true,
        ...
        ConfigElementsTab:Input({
            Flag = "InputTest", Title = "Input", Desc = "Input Description",
            Value = "Default value", InputIcon = "bird", Type = "Input",
            Placeholder = "Enter text...",
            Callback = function(input) print("Text entered: " .. input) end,
        })
        ConfigElementsTab:Space()
        ConfigElementsTab:Keybind({
            Flag = "KeybindTest", Title = "Keybind", Desc = "Keybind to open ui",
            Value = "G",
            Callback = function(v) Window:SetToggleKey(Enum.KeyCode[v]) end,
        })
        ConfigElementsTab:Space()        ConfigElementsTab:Slider({
            Flag = "SliderTest", Title = "Slider", Step = 1,
        ...
        ConfigElementsTab:Toggle({
            Flag = "ToggleTest", Title = "Toggle Panel Background",
            Value = not Window.HidePanelBackground,
            Callback = function(state) Window:SetPanelBackground(state) end,
        })
        ConfigElementsTab:Toggle({
            Flag = "ToggleTest", Title = "Toggle", Desc = "Toggle Description",
            Value = false,
    ...
    do
        ...
        local ConfigNameInput = ConfigTab:Input({
            Title = "Config Name", Icon = "file-cog",
        ...
        local AllConfigsDropdown = ConfigTab:Dropdown({
            Title = "All Configs", Desc = "Select existing configs",
        ...
        ConfigTab:Space()        ConfigTab:Button({
            Title = "Save Config", Icon = "", Justify = "Center",
            Callback = function()
        ...
        ConfigTab:Space()
        ConfigTab:Button({
            Title = "Load Config", Icon = "", Justify = "Center",
            Callback = function()
        ...
        ConfigTab:Space()
        ConfigTab:Button({
            Title = "Print AutoLoad Configs", Icon = "", Justify = "Center",
            Callback = function() print(HttpService:JSONDecode(ConfigManager:GetAutoLoadConfigs())) end,
        })
    end
end
...
do
    ...
    local Response = WindUI.cloneref(game:GetService("HttpService"))
        :JSONDecode(WindUI.Creator.Request and WindUI.Creator.Request({
            ...
            Headers = { ["User-Agent"] = "WindUI/Example", ["Accept"] = "application/json" },
        }).Body or "{}")

    local DiscordTab = OtherSection:Tab({ Title = "Discord", Border = true })
    ...
    if Response and Response.guild then
        ...
        DiscordTab:Paragraph({
            ...
            ImageSize = 48,
            Buttons = {
                { Title = "Copy link", Icon = "link", Callback = function() setclipboard("https://discord.gg/" .. InviteCode) end },            },
        })
    elseif RunService:IsStudio() or not writefile then
        DiscordTab:Paragraph({
            Title = "Discord API is not available in Studio mode.",
            TextSize = 20, Justify = "Center",
            Image = "solar:info-circle-bold", Color = "Red",
            Buttons = {
                { Title = "Get/Copy Invite Link", Icon = "link",
                  Callback = function()
                      if setclipboard then setclipboard("https://discord.gg/" .. InviteCode)
                      else WindUI:Notify({ Title = "Discord Invite Link", Content = "https://discord.gg/" .. InviteCode }) end
                  end
                },
            },
        })
...
-- /* Example Tab with cascading dropdowns */ --
local Tabs = {
    ExampleTab = Window:Tab({ Title = "Example Tab", Icon = "bird" }),
}

local dropdownA
local LargeListA = { "All" }
for i = 2, 100 do table.insert(LargeListA, "Item A" .. i) end

local LargeListB = {}
for i = 1, 10 do table.insert(LargeListB, "Data B" .. i) end
